...
[ ] 베팅 방식을 선택합니다.
    ...
    - 입력된 방식에 따라, 다음의 말 선택창에서 안내 메시지를 다르게 출력합니다.
...
[ ] 베팅 금액을 결정합니다.
    ...
    - 이전에 입력받은 유저가 선택한 베팅 방식, 말 정보와 함께 BetProcessor 객체를 얻어냅니다.
...
[ ] 경주 순위는 다음과 같은 과정으로 정합니다.
    ...
    - 필드값이 동일하다면, 말 이름의 가나다 순으로 결정합니다.
        - 반드시 서로 다른 값이 나오도록 설정하는 방법이 있을 지 고민중입니다.

[ ] 레이싱의 결과와 유저 정보, 유저의 베팅 정보를 이용하여, 베팅 전략에 따라 결과를 구한 뒤, 새로운 User 객체로 반환합니다.

    - BetProcessor의 updateUserWithResult 메서드를 호출합니다.
    - updateUserWithResult 메서드는 BettingStrategy의 디폴트 메서드인 updateUserBalance를 호출합니다.
    - updateUserBalance 안의 getReward 메서드는 각 구현 클래스에서 재정의하며,
